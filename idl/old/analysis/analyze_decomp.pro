...
for i=0,n_elements(fwhm_orig[0,*,0,0])-1 do begin
  if i eq 0 then begin
    plot, noise_vector, total((abs(fwhm_calc-fwhm_orig)/(fwhm_orig))[*,i,*,0],3)/n, $
      ...
      yrange=[0,10],$
      charsize=2., thick=2, psym=-4
    xyouts, 0.8, 0.93, 'Blob size: 10mm', /normal
  endif else begin
    oplot, noise_vector, total((abs(fwhm_calc-fwhm_orig)/(fwhm_orig))[*,i,*,0],3)/n, color=80+i*20, psym=-4
...
for i=0,n_elements(fwhm_orig[0,*,0,1])-1 do begin
  if i eq 0 then begin
    ...
    oplot, noise_vector, total((abs(fwhm_calc-fwhm_orig)/(fwhm_orig))[*,i,*,1],3)/n, color=80+i*20, psym=-4
    xyouts, 0.8, 0.93-i*0.03, 'Blob size: '+strtrim((i+2)*5,2)+'mm', /normal, color=80+i*20
  endelse
endfor
...
for i=0,n_elements(fwhm_orig[0,*,0,1])-1 do begin
  if i eq 0 then begin
    ...
    oplot, noise_vector, total((abs(pos_calc-pos_orig)/(pos_orig))[*,i,*,0],3)/n, color=80+i*20, psym=-4
    xyouts, 0.8, 0.93-i*0.03, 'Blob size: '+strtrim((i+2)*5,2)+'mm', /normal, color=80+i*20
  endelse
endfor
...
for i=0,n_elements(fwhm_orig[0,*,0,1])-1 do begin
  if i eq 0 then begin
    ...
    oplot, noise_vector, total((abs(pos_calc-pos_orig)/(pos_orig))[*,i,*,1],3)/n, color=80+i*20, psym=-4
    xyouts, 0.8, 0.93-i*0.03, 'Blob size: '+strtrim((i+2)*5,2)+'mm', /normal, color=80+i*20
  endelse
endfor

...
def fluctuation_matrix_test(rad_size=10.,
                            vert_size=10.,
                            rad_pos=2200.,
                            vert_pos=15.,
                            fluct_amp=0.05,
                            ...
                            moving=False,
                            vx=100, vy=50, v_size=50., rot_freq=100.,
                            hole_size=10., hole_rad_pos=2200., hole_vert_pos=15.,
                            hole_fluct_amp=-0.02,
                            hole_vx=-50.,
                            hole_vy=100.,
                            time_win=1e-3,time_vec=None,
                            sampling_time=0.5e-6, blob_density=1e20):
    ...
    if (moving is None) :
        ...
        return_matrix=np.zeroes(n_vert,n_rad,n_time)
        blob_density=np.zeroes(n_time) # Gaussian integral divided by the ellipse defined by the sigma_r, sigma_z of the Gaussian (surface density)
        ...
        for i in range(0, n_time):
            ...
            for j in range(0, n_vert):
                for k in range(0, n_rad):
                    ...
                    if not i: zrange=[hole_fluct_amp, fluct_amp]
                    if (plot is None) :
                        plt.contourf(return_matrix[:,:,i],
                                     ...
                                     nlevel=21)
                        #TODO: plt.zlim
                        #TODO: plots, reform(spatial_pos[*,*,0]),reform(spatial_pos[*,*,1]), psym=4

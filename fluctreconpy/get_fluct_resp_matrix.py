    ...
    if (test is None) :
        ...
        for i_rad in range(0, nr):
            for j_vert in range(0, nz):
                tauvec[i_rad,j_vert]=(np.tanh((spatial_pos[j_vert,nr-1,0]-spatial_pos[j_vert,i_rad,0])/20.-2.)+1.)*5.*1e-9
        ...
        for k in range(0, nz):
            for j in range(0, nr):
                for i in range(j, nr):
                    matrix[k,i,k,j]=1e-9/tauvec[i,k]*np.exp(-((i-j)*0.01)/(v*tauvec[i,k]))

...
pro test_fluct_decomp, nocalibrate=nocalib, k_factor=k_factor, $
    ...
    blob_density_orig=blob_density_orig, blob_density_samp=blob_density_samp
        ...
        default, n_rad_meas, len(spatial_pos[0,*,0])
        default, n_vert_meas, len(spatial_pos[*,0,0])
        default, n_rad_calc, len(spatial_pos[0,*,0])
        default, n_vert_calc, len(spatial_pos[*,0,0])
        default, nocalib, 1.
        ...
        s_vector=numpy.zeroes(n_vert_meas,n_rad_meas)
        n_vector=numpy.zeroes(n_vert_calc,n_rad_calc)
        n0=1. #Characteristic density
        error_vector=numpy.zeroes(n_vert_meas,n_rad_meas) #square of the standard error
        if (moving is not None) :
            ...
            n_vector=fluctuation_matrix_test(rad_pos=blob_pos[0],vert_pos=blob_pos[1], /moving, vert_size=blob_size[1], rad_size=blob_size[0], $
                ...
                loadct, 5
                blob_density_samp=numpy.zeroes(len(time_vec))
                for i in range[0, len(time_vec)]:
                    ...
                    n_vector_calc[*,*,i]=calculate_decomposition(light_profile=s_vector, error_profile=error_vector, m_matrix=m_matrix, spatial_pos=spatial_pos, $
                        ...
                        if (visual is None):                                            
                            if i == 0: range=range[[n_vector,n_vector_calc]]
                            ...
                            if i == 0: zmax_light=max([max(s_vector_ref),max(m_matrix_ref ## p_vector)])
                            if i == 0: zmin_light=min([min(s_vector_ref),min(m_matrix_ref ## p_vector)])
                            erase
